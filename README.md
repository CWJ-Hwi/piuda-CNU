...
- 안심/주의/위험/긴급 4단계 상태 및 보호자 알림 기록
...
- 보호자가 연령·성별을 선택하고 질병·장애·대화 선호를 문장으로 기록하는 AI 맞춤 정보
- 낮 시간 장시간 무활동 시 사용자에게 먼저 묻고, 30초 미응답 시에만 보호자에게 알림
- `/demo` 발표 제어실의 기본 상태·식사 지연·장시간 비움 3개 핵심 장면
- 사용자 버튼·위험 감지로 생성되는 보호자 확인 알림과 30초 중복 방지
- Raspberry Pi에서 실측해 선택한 한국어 특화 HyperCLOVA X SEED 1.5B Q4, 사용자 맞춤 정보·최근 대화 기억과 즉시 안전 폴백
...
평소에는 100점에서 시작하고 감지된 신호만큼 감점합니다. 76~100 안심, 51~75 주의, 21~50 위험, 0~20 긴급으로 분류합니다. 이 판정은 의료 진단이 아니라 보호자의 확인 우선순위를 정하는 보조 정보입니다.
...
Pi가 만든 `PIUDA-CNU` Wi-Fi에 ESP32와 발표 기기를 연결한 뒤 트리거 담당자는 `http://192.168.4.1:8080/demo`, 사용자 담당자는 Raspberry Pi 화면, 보호자 담당자는 `http://192.168.4.1:8080/caregiver`를 엽니다. 제어실의 **기본 상태·식사 지연·장시간 비움** 세 장면은 2초 안에 자동 반영됩니다. 장시간 비움에서는 사용자와 보호자 화면 모두에 위험 팝업이 표시됩니다.
